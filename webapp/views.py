...
def index(request):
    ...
    services_index = Service.objects.all()
    developers = Developer.objects.all()
    faqs = FAQ.objects.all()
    latest_posts = BlogPost.objects.order_by('-post_date')[:6]
    return render(request, 'webapp/index.html',
                  {'site': site, 'developers': developers, 'services': services_index, 'faqs': faqs,
...
def about(request):
    ...
    return render(request, 'webapp/about.html',
                  {'portfolio': portfolio, 'page_range': page_range, 'current_page': current_page, 'review': review})
